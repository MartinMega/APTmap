function [HydrogenTab] = BuildHydrogenTable(FileTab, voltagebins, Hrange, H2range, ...
    MinCounts_voltagebin, MinHinBin, doItParallel)
...
results = cell(cell(height(FileTab),length(tab_Varnames)));


ti = tic;


%loop through FileTab
if doItParallel
    parfor k = 1:height(FileTab)
        newrow_cell = readFilePrepareTableRow_BulidHydrogenTable(FileTab(k,:), voltagebins, MinCounts_voltagebin, MinHinBin, Hrange, H2range, binw_noisecorr, conf_noisecorr);
        results(k,:) = newrow_cell;
        elapsed = toc(ti);
        fprintf("Processed: %u | Remaining: %u | Elapsed: %.1f s  (may be out of order when working parallel) \n", k, height(FileTab)-k, elapsed);
    end
else
